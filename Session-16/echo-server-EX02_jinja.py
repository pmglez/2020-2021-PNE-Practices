...
class TestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/":
            contents = Path(HTML_ASSETS + "form-EX02.html").read_text()
        elif self.path.startswith("/echo"):
            message = parse_qs(urlparse(self.path).query)["msg"][0]
            print("Message is:", message)
            if "capital=on" in self.path:
                contents = read_template_html_file(HTML_ASSETS + "template_jinja.html").render(msg=message.upper())
            else:
                contents = read_template_html_file(HTML_ASSETS + "template_jinja.html").render(msg=message)
        else:
